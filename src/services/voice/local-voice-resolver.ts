...
export class SQLiteVoiceResolver implements VoiceResolver {
  constructor(
    ...
    private readonly config: ResolverConfig = DEFAULT_VOICE_RESOLVER_CONFIG,
  ) {}
  ...
  async resolve(request: VoiceResolveRequest): Promise<VoiceResolution> {
    ...
    let lastUnrecognised: VoiceResolution | undefined;
    for (const hypothesis of hypotheses.slice(0, 5)) {
      ...
      if (result.kind === "unrecognized") lastUnrecognised = result;
    }
    return (
      lastUnrecognised ?? {
        kind: "unrecognized",
