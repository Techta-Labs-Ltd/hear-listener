...
export function runCommand(
  ...
  switch (command.type) {
    ...
    case "speed":
      ...
      return playbackSpeedSetMessage(command.multiplier);
    case "speedStep": {
      const current = s.playback.speed ?? 1;
      const currentIndex = PLAYBACK_SPEED_OPTIONS.indexOf(current);
      const targetIndex = Math.max(
        0,
        Math.min(
          PLAYBACK_SPEED_OPTIONS.length - 1,
          currentIndex + (command.direction === "up" ? 1 : -1),
        ),
      );
      ...
      return playbackSpeedSetMessage(PLAYBACK_SPEED_OPTIONS[targetIndex]);
    }
    case "saveCurrent": {
...
function playbackSpeedSetMessage(speed: SpeedMultiplier): string {
  return speed === 1
...
class AppVoiceExecutor implements VoiceExecutor {
  ...
  private valid(invocation: VoiceInvocation) {
    return (
      VOICE_EXECUTOR_KEYS.has(invocation.executorKey) &&
