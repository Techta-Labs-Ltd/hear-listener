...
import {
  VOICE_DATABASE_BUSY_RETRY_DELAY_MS,
  VOICE_DATABASE_MAX_BUSY_RETRIES,
  VOICE_DATABASE_MAX_CACHE,
  VOICE_DATABASE_NAME,
  VOICE_DATABASE_OPTIONS,
  VOICE_SCHEMA_VERSION,
} from "@/constants/voice-database";
import type {
  BiasTermInput,
  EntityCandidate,
  EntityType,
...
import { buildFtsMatch } from "@/utils/voice/matching/fts-query";
import { phoneticCodeScore } from "@/utils/voice/matching/phonetic";
import { mergeCandidates } from "@/utils/voice/matching/candidate-merge";
...
async function openVoiceDatabase() {
  await importDatabaseFromAssetAsync(VOICE_DATABASE_NAME, {
    assetId: appAssets.database.voiceSeed,
  });
  let database = await openDatabaseAsync(
    VOICE_DATABASE_NAME,
    VOICE_DATABASE_OPTIONS,
  );
  try {
...
async function configureDatabase(database: SQLiteDatabase) {
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      if (
        attempt >= VOICE_DATABASE_MAX_BUSY_RETRIES ||
        !isBusyError(error)
      ) {
        throw error;
      }
      await new Promise((resolve) =>
        setTimeout(resolve, VOICE_DATABASE_BUSY_RETRY_DELAY_MS * attempt),
      );
    }
  }
}
...
async function deleteDatabaseFiles() {
  ...
  for (const name of [
    VOICE_DATABASE_NAME,
    `${VOICE_DATABASE_NAME}-wal`,
    `${VOICE_DATABASE_NAME}-shm`,
    `${VOICE_DATABASE_NAME}-journal`,
  ]) {
    try {
...
export async function migrateVoiceDatabase(
  ...
  await database.withExclusiveTransactionAsync(async (tx) => {
    await tx.execAsync(`
      ...
      DROP TABLE IF EXISTS voice_entity_fts;
...
class SQLiteVoiceRepository implements VoiceEntityRepository {
  ...
  async searchEntities(
    ...
    const exactRows = await database.getAllAsync<VoiceCandidateRow>(
    ...
    const exactCanonical = await database.getAllAsync<VoiceCandidateRow>(
    ...
    if (match) {
      const ftsRows = await database.getAllAsync<VoiceCandidateRow>(
        `SELECT e.entity_id AS entityId, e.entity_type AS entityType,
    ...
    if (grams.length) {
      ...
      const trigramRows = await database.getAllAsync<
        VoiceCandidateRow & { matched: number }
      >(
        `SELECT e.entity_id AS entityId, e.entity_type AS entityType,
    ...
    const phoneticRows = await database.getAllAsync<VoiceCandidateRow>(
      `SELECT e.entity_id AS entityId, e.entity_type AS entityType,
              e.canonical_name AS canonicalName, NULL AS matchedAlias,
              e.popularity, e.metadata_json AS metadataJson,
    ...
    const aliasPhoneticRows = await database.getAllAsync<VoiceCandidateRow>(
      `SELECT e.entity_id AS entityId, e.entity_type AS entityType,
              e.canonical_name AS canonicalName, a.normalized_alias AS matchedAlias,
              e.popularity, e.metadata_json AS metadataJson,
    ...
    if (this.cache.size > VOICE_DATABASE_MAX_CACHE)
  ...
  async getEntity(type: EntityType, id: string): Promise<VoiceEntity | null> {
    ...
    const row = await database.getFirstAsync<
      VoiceCandidateRow & { revision: string }
    >(
      `SELECT entity_id AS entityId, entity_type AS entityType,
  ...
  async getEntitiesByIds(refs: EntityRef[]): Promise<VoiceEntity[]> {
    ...
    const rows = await database.getAllAsync<
      VoiceCandidateRow & { revision: string; normalizedName: string }
    >(
      `SELECT entity_id AS entityId, entity_type AS entityType,
              canonical_name AS canonicalName, normalized_name AS normalizedName,
              primary_metaphone AS primaryMetaphone, secondary_metaphone AS secondaryMetaphone,
